...
![Three pairs of lizard species co-speciating when their island is fragmented](./images/div-island-cartoon-shared-event-labels.png "Lizards co-speciating when their island is fragmented"){#fig-divcartoon fig-alt="Three pairs of lizard species co-speciating when their island is fragmented" fig-align="center" width=70%}
...
Below, @fig-divmodels illustrates these 5 models, where $\tau$ represents
independent divergence-time parameters.

::: {#fig-divmodels layout-ncol=2 layout-nrows=3}

![All 3 pairs co-diverge](./images/div-model-111-labels.svg "Three pairs of lizard species co-speciating"){fig-alt="Three pairs of lizard species co-speciating" fig-align="center"}

![Pair 1 diverges independently](./images/div-model-311-labels.svg "Two pairs of lizard species co-speciating"){fig-alt="Two pairs of lizard species co-speciating" fig-align="center"}

![Pair 2 diverges independently](./images/div-model-131-labels.svg "Two pairs of lizard species co-speciating"){fig-alt="Two pairs of lizard species co-speciating" fig-align="center"}

![Pair 3 diverges independently](./images/div-model-113-labels.svg "Two pairs of lizard species co-speciating"){fig-alt="Two pairs of lizard species co-speciating" fig-align="center"}

![All 3 pairs diverge independently](./images/div-model-213-labels.svg "Three pairs of lizard species speciating independently"){fig-alt="Three pairs of lizard species speciating independently" fig-align="center"}

Five possible divergence models for 3 pairs of lizard species
:::
...
-   $M_1$: All values are equally probable; $\theta \sim \text{Beta}(1,1)$
-   $M_2$: It might be a trick coin, but we don't know if it's weighted toward heads or tails; $\theta \sim \text{Beta}(0.6,0.6)$
-   $M_3$: The coin is probably fair; $\theta \sim \text{Beta}(5,5)$
-   $M_4$: The coin is probably weighted toward tails; $\theta \sim \text{Beta}(1,5)$

If we happen to flip 50/100 heads, @fig-coinflip (from @Oaks2018marginal) shows
what our posterior distributions of $\theta$ will look like, along with the
marginal likelihoods of the four models.

![Results of our hypothetical coin-flip experiment. From @Oaks2018marginal.](./images/coin-flip.jpg "Results of our coin-flipping experiment"){#fig-coinflip fig-alt="Results of our coin-flipping experiment" fig-align="center" width=90%}
...
$$
p(D) = \int_{\tau_1}\int_{\tau_2}\int_{\tau_3}\int_{\mu} p(D | \tau_1, \tau_2, \tau_3) p(\tau_1 | \mu) p(\tau_2 | \mu) p(\tau_3 | \mu) p(\mu) d\tau_1 d\tau_2 d\tau_3 d\mu
\text{.}
$$

Notice that $\mu$ (the mean of the divergence-time prior) is one-level removed
from the data.
...
Also notice that we have the new term $p(\mu)$ in our model.
This is the prior probability of $\mu$, the mean of the divergence-time prior
distribution.
When we want to estimate a parameter of a prior distribution, we have to put a
prior on it.

Above, we use the mean of the prior distribution on divergence times as an
example.
In theory, we can do this for any parameter of the prior distribution
on divergence times
(*e.g.*, shape, scale, mean, standard deviation, min, max, etc.).

### What's "hyper"? Terminology of hierarchical Bayesian models

In the parlance of Bayesian statistics,
when we estimate the parameter of a prior distribution, we
call it a ***hyperparameter***.
The prior distribution we place on a hyperparameter (i.e., the prior on the
parameter of a prior) is called a ***hyperprior***.
For this project, we will be exploring hyperpriors on divergence times in
the
[ecoevolity](https://phyletica.org/ecoevolity)
software pacakge.
...
In comparative biogeographic methods like msbayes and ecoevolity,
prior distributions on divergence times with lots of uncertainty can
create a strong penalty against models with more divergence-time parameters,
and thus cause a downward bias in the number of divergence events across the
taxa being compared.
For this project, we will explore whether a hierarchical Bayesian approach can
mitigate such a bias.
Specifically, we will explore using hyperpriors on the parameters of the prior
on divergence times to see if the model can learn about the divergence-time
prior during the analysis and better calibrate the "penalty" against adding
divergence-time parameters to the model.
